...
let rankingLoadError = false;
let rankingScriptElement = null;

window.levelUpRankingCallback = function(payload) {

  rankingLoaded = true;
  rankingLoadError = false;
  ...
  if (
    ...
    dailyRanking = payload.ranking.slice(0, 5);
  } else {
    dailyRanking = [];
  }
...
function loadDailyRanking() {
  ...
  // Quitamos la consulta anterior si todavía existe.
  if (
    rankingScriptElement &&
    rankingScriptElement.parentNode
  ) {
    rankingScriptElement.parentNode.removeChild(rankingScriptElement);
  }
  ...
  script.src =
    STATS_ENDPOINT +
    "?action=ranking" +
    "&callback=levelUpRankingCallback" +
    "&t=" + Date.now();
...
function updateCollectibles() {
  ...
  if (
    ...
    // Los coleccionables siguen apareciendo con regularidad,
    // pero dejamos bastante más espacio entre uno y otro.
    if (frenzyActive) {
      nextCollectible =
        150 +
...
// ==========================================
// CORAZONES GARANTIZADOS DESDE NIVEL 5
// ==========================================

function createGuaranteedHeart() {
  ...
  const y =
    heartHeights[
      Math.floor(
        Math.random() * heartHeights.length
      )
    ];
